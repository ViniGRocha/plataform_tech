...
function renderQuestion() {
    ...
    q.options.forEach((opt) => {
        ...
        container.innerHTML += `
    <label class="block">
        <input type="radio" name="option" value="${opt.value}" ${checked}
            class="hidden peer">
        ...
        <div class="p-5 rounded-xl border border-gray-300 bg-white
            ...
            <!-- ÍCONE CHECK -->
            <div class="absolute right-4 top-4 opacity-0 
            ...
            <div class="flex items-center gap-4">
                ...
                <!-- TEXTO -->
                <p class="text-gray-700 peer-checked:text-tech-primary font-medium">
                    ${opt.label}
                </p>
            </div>
...
`;
    ...
    if (current === 0) {
        prev.disabled = true;
        prev.classList.add("opacity-30", "cursor-not-allowed");
    } else {
        prev.disabled = false;
        prev.classList.remove("opacity-30", "cursor-not-allowed");
    }

    // ANIMAÇÃO DE ENTRADA
    const box = document.getElementById("quiz-box");
    box.classList.remove("animate-fadeInUp");
    setTimeout(() => box.classList.add("animate-fadeInUp"), 10);
}
